...
fprintf(fid,'# This file includes the original data computed by the BLE code in the BLE computating mesh \n #All the data are dimensional\n');
fprintf(fid,'VARIABLES = "s","x/c", "y", "Ro","U", "V", "W", "T" \n  ZONE I=    %d  J = %d  F=POINT \n',n-6,(N+1));
for j=1:N+1
    for i=7:n
         fprintf(fid, '%20.16f %20.16f %20.16f %20.16f %20.16f %20.16f %20.16f %20.16f\n', S(i),xc(i), yout(j,i) ,Ro3(j,i),U3(j,i),V3(j,i),W3(j,i),T3(j,i));
%          
    end
end
fclose(fid);
...
for i=1:1:NN*n
    ...
    fprintf(fid5, '%17.15e %17.15e %17.15e %17.15e 0.000000\n', T4(i,:));
end
...
fid = fopen('../output/BASEFLOW/KOUT.dat', 'wt');
for i=1:1:n
    fprintf(fid, '%17.15e\n', -Curve(i)*deta0);
end
fclose(fid);
...
fprintf(fid,'# This file includes the data which will be used by LST and NPSE code \n # All this data have already interplated into the NPSE mesh \n # All this data are dismensionless\n');
...
fid3 = fopen('test/BoundaryLayerThickness.dat','wt');
fprintf(fid3,'variables = x/c DisplaceThickness(mm) MomentumThickness(mm) \n ZONE T = Boune=daryLayerThickness I = %d  \n',n-6);
INym = zeros(NN,1);
DisplaceThickness = zeros(1,n);
MomentumThickness = zeros(1,n);
for j = 2:(NN-1)
    INym(j) = (ym(j+1) - ym(j-1))/2;
end
INym(1) = (ym(2) - ym(1))/2; 
INym(NN) = (ym(NN) - ym(NN-1))/2; 
for i = 7:n
    ...
    DisplaceThickness(i) = (ones(NN,1)-Ut)'*INym;
    MomentumThickness(i) = (Ut.*(ones(NN,1)-Ut))'*INym;
    fprintf(fid3,'%17.15e ',xc(i),DisplaceThickness(i)*deta0*1000,MomentumThickness(i)*deta0*1000);
     fprintf(fid3,'\n');
    for j = 1:NN
...
fprintf(fid_set,' Ureff  Treff  Roreff  mureff  Re0  deta0  beta   omega   i  cord_length\n');
fprintf(fid_set,'%20.16f %20.16f %20.16f %20.16f %20.16f %20.16f XXXX XXXX XXXX %20.16f',Ureff,Treff,Roreff,mureff,Re0,deta0,c);
fclose(fid_set);
%生成LST需要的网格文件******************************************
