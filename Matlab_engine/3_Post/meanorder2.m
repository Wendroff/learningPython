function   [Fai,H,Lbs]=meanorder2(Fai,i,N,LN,IN,y)
...
%    pause
     ...
     for itr=1:1:100000000
        ...
        %%%%%%%%%%%%%
         % x equation
         D22=S(i)*diag(Ut)*(3/2/dx)+diag(Vt)*LN+diag(beta1)*diag(Ut)*eye(N+1)-LN*diag(mub)*LN;  % order
         
         D24=-diag(beta1)*eye(N+1);
         
         % z equation
         D33=S(i)*diag(Ut)*(3/2/dx)+diag(Vt)*LN-LN*diag(mub)*LN;                                % order
         ...
         D44=S(i)*diag(Ut)*(3/2/dx)+diag(Vt)*LN-1/Pr*LN*diag(kb)*LN;                            % order
         %%%%%%%%%%%%%%
         DV0=eye(N+1);
         for j=2:1:N+1
             DV0(j,j-1)=-1;
        ...
        for j=2:1:N
             ...
             Lbs=[Lbs;
                  DV0(j,:)       IN(j,:)*(S(i)*(3/2/dx)+0.5*(1+beta0))   zeros(1,N+1)   zeros(1,N+1); % order
                  zeros(1,N+1)   D22(j,:)                                zeros(1,N+1)   D24(j,:);
                  zeros(1,N+1)   zeros(1,N+1)                            D33(j,:)       zeros(1,N+1);
                  zeros(1,N+1)   D42(j,:)                                D43(j,:)       D44(j,:);  ];
             F=[F;
                IN(j,:)*S(i)*(4/2/dx*U(:,i-1)-1/2/dx*U(:,i-2));       % order
                S(i)*Ut(j)*(4/2/dx*U(j,i-1)-1/2/dx*U(j,i-2));         % order
                S(i)*Ut(j)*(4/2/dx*W(j,i-1)-1/2/dx*W(j,i-2));         % order
                S(i)*Ut(j)*(4/2/dx*T(j,i-1)-1/2/dx*T(j,i-2));];       % order
        end
        ...
        j=N+1; % boundary layer
         ...
         for j=2:1:N
             ...
             RU1(1,j-1)=0.5*(y(j)-y(j-1))*(S(i)*(3/2/dx)+0.5*(1+beta0));   % order
             RU1(1,j)=0.5*(y(j)-y(j-1))*(S(i)*(3/2/dx)+0.5*(1+beta0));     % order
             
             %  x equation
             RU2=zeros(1,N+1);  RT2=zeros(1,N+1);
             
             RU2(1,j)=S(i)*(3/2/dx)*Ut(j)+beta1*Ut(j);                     % order
